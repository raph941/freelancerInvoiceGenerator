...
var serverFormData = null //holds the form data to be sent to the server
...
var collectSendData = () => {
    ...
    var alreadyAddedDiv = document.querySelectorAll("#rowsContainer > div");
        ...
        $.ajax({
            url: window.location.host + "/invoice/save/",
            method: "POST",
            data: {
                'due_date': values.get("dueDate"),
                'invoice_date': values.get("invoiceDate"),
                'company_name': values.get("companyName"),
                'company_address': values.get("companyAddress"),
                'company_country': values.get("companyCountry"),
                'company_city': values.get("companyCity"),

                'client_name': values.get("clientName"),
                'client_address': values.get("clientAddress"),
                'client_country': values.get("clientCountry"),
                'client_city': values.get("clientCity"),

                'invoice_items': JSON.stringify(listItemSend),
                'subtotal': document.querySelector('#subTotal').innerText,
                'sales_tax': document.querySelector('#taxes').innerText,
                'note': values.get("feedback"),
                'terms': values.get("paymentMethod"),
                
                'csrfmiddlewaretoken': $('#data-store').attr('csrf'),
            },
